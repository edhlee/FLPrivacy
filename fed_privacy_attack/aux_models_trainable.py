...
def i3d_v2():
    #inputs = tf.keras.layers.Input(shape=[24, 156, 156,1])
    inputs = tf.keras.layers.Input(shape=[24, 256, 256,1])
#    inputs = tf.keras.layers.Input(shape=[24, 240, 240,1])

    inputs_ = tf.keras.layers.Concatenate(axis=4)([inputs, inputs, inputs])
    # inputs_ = tf.ensure_shape(inputs_, [1, 24, 256, 256, 3])
    print("layerb1: " , inputs_.shape)
 ...
 #   FRAME_WIDTH=240
    ...
    hidden = tf.keras.layers.Activation('softmax', dtype='float32')(hidden)
